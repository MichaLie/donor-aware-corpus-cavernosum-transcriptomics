...
full_results_file <- file.path(root, "results", "tables", "GSE206528_pseudobulk_edger", "all_pseudobulk_edgeR_results.tsv.gz")
out_dir <- file.path(root, "results", "tables", "GSE206528_pseudobulk_edger_lodo")
...
candidate_keys <- unique(candidates[, .(cell_type, comparison, gene_symbol, full_logFC = logFC, full_PValue = PValue, full_FDR = FDR)])

comparison_specs <- list(
  organic_ED_vs_normal = list(
    group_column = "primary_group",
    levels = c("normal", "organic_ED")
  ),
  non_diabetic_ED_vs_normal = list(
    group_column = "ed_subgroup",
    levels = c("normal", "non_diabetic_ED")
  ),
  diabetic_ED_vs_normal = list(
    group_column = "ed_subgroup",
    levels = c("normal", "diabetic_ED")
  )
)
...
for (comparison_name in names(comparison_specs)) {
  ...
  for (cell_type in sort(unique(meta$major_cell_type))) {
    cell_type_value <- cell_type
    cell_meta <- meta[
      meta$major_cell_type == cell_type &
        meta$n_cells >= min_cells &
        meta[[spec$group_column]] %in% spec$levels,
      ,
      drop = FALSE
    ]
    cell_counts <- counts[, rownames(cell_meta), drop = FALSE]

    cell_candidates <- candidate_keys[cell_type == cell_type_value & comparison == comparison_name]
    if (nrow(cell_candidates) == 0) {
      next
    }

    for (leave_out_donor in sort(unique(cell_meta$donor_id))) {
      lodo_meta <- cell_meta[cell_meta$donor_id != leave_out_donor, , drop = FALSE]
      lodo_counts <- cell_counts[, rownames(lodo_meta), drop = FALSE]
      ...
      if (is.null(res)) {
        lodo_diagnostics[[paste(comparison_name, cell_type, leave_out_donor, sep = "__")]] <- diagnostic
        next
      }
      ...
      merged <- merge(cell_candidates, res_dt, by = "gene_symbol", all.x = TRUE)
      merged[, `:=`(
        cell_type = cell_type,
        comparison = comparison_name,
        leave_out_donor = leave_out_donor,
      ...
      keep_cols <- c(
        "cell_type", "comparison", "gene_symbol", "leave_out_donor",
      ...
      lodo_results[[paste(comparison_name, cell_type, leave_out_donor, sep = "__")]] <- merged[, ..keep_cols]
    }
  }
}
...
), by = .(cell_type, comparison, gene_symbol)]
...
setorder(summary_df, cell_type, comparison, full_FDR, full_PValue, gene_symbol)
...
setorder(overview_df, cell_type, comparison)
fwrite(overview_df, file.path(out_dir, "candidate_lodo_overview.tsv"), sep = "\t")

message("Wrote leave-one-donor-out candidate results to: ", out_dir)
