#!/usr/bin/env python3
"""Summarize edgeR pseudobulk results with simple donor-direction checks."""
...
PSEUDOBULK_DIR = ROOT / "data" / "processed" / "GSE206528" / "pseudobulk"
RESULT_DIR = ROOT / "results" / "tables" / "GSE206528_pseudobulk_edger"

COUNTS_FILE = PSEUDOBULK_DIR / "GSE206528_pseudobulk_counts_by_donor_celltype.tsv.gz"
META_FILE = PSEUDOBULK_DIR / "GSE206528_pseudobulk_sample_metadata.tsv"
RESULTS_FILE = RESULT_DIR / "all_pseudobulk_edgeR_results.tsv.gz"
...
def main() -> None:
    ...
    for row in candidates.itertuples(index=False):
        grouping = COMPARISON_GROUPS[row.comparison]
        group_column, control_label, disease_label = grouping

        sample_meta = meta.loc[
            (meta["major_cell_type"] == row.cell_type)
            & (meta["n_cells"] >= 50)
        ...
        disease_mean = float(disease_values.mean())
        ...
        rows.append(
            {
                ...
                "control_mean_logCPM": control_mean,
                "disease_mean_logCPM": disease_mean,
                "disease_minus_control_mean_logCPM": disease_mean - control_mean,
    ...
    consistency = consistency.sort_values(
        ["cell_type", "comparison", "FDR", "PValue", "gene_symbol"]
    )
    consistency.to_csv(
    ...
    overview = (
        consistency.assign(
            ...
            is_directionally_consistent=lambda df: df[
                "disease_fraction_consistent_vs_control_median"
            ]
            >= 0.8,
            is_fdr_0_05_directionally_consistent=lambda df: df["is_fdr_0_05"]
            & df["is_directionally_consistent"],
        )
        .groupby(["cell_type", "comparison"], as_index=False)
        .agg(
            n_FDR_0_10=("gene_symbol", "size"),
            n_FDR_0_05=("is_fdr_0_05", "sum"),
            n_FDR_0_05_directionally_consistent=(
                "is_fdr_0_05_directionally_consistent",
                "sum",
            ),
        )
    )
    ...
    print("Wrote directional consistency summaries to:", RESULT_DIR)
